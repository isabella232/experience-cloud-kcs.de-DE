...
Der Ausführungsmodus &quot;Überprüfen&quot;von [!DNL oak-run] kann verwendet werden, um die letzte zweifelsfrei funktionierende Revision eines Segmentspeichers zu ermitteln.  Dies kann verwendet werden, um einen beschädigten Segmentspeicher manuell auf die neueste gute Revision zurückzusetzen.
...
1. Repository durch Bearbeiten auf diese Revision zurücksetzen `./crx-quickstart/repository/segmentstore/journal.log` und das Löschen aller Zeilen nach der Zeile, die die letzte gute Revision enthält.

1. Alle entfernen `./crx-quickstart/repository/segmentstore/\*.bak` Dateien.
...
1. Entfernen Sie alle identifizierten beschädigten Knoten, die in der Ausgabe des letzten Befehls mit `rmNodes.groovy`
   ...
   Führen Sie die `rmNode` Befehl zum Entfernen des beschädigten Knotens, ersetzen Sie `/path/to/corrupt/node` mit dem Pfad zum beschädigten Knoten, den Sie entfernen müssen.
