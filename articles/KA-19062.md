...
Verwenden Sie zunächst das Tool &quot;oak run&quot;, ein ausführbares jar1-Tool, das alles enthält, was Sie für eine einfache Oak-Installation und die Ausführung von oak-bezogenen Vorgängen benötigen.
...
3. Führen Sie die Konsistenzprüfung aus, java -Xmx6000m -jar oak-run-\*.jar check —bin=-1 /path/to/crx-quickstart/repository/segmentstore Dies durchsucht die Revisionen rückwärts, bis es eine konsistente findet: Suchen Sie nach Nachricht wie folgt:
...
1. Entfernen Sie alle identifizierten beschädigten Knoten, die in der Ausgabe des letzten Befehls mit rmNodes.groovy aufgeführt sind.
...
1. Führen Sie eine Checkpoint-Bereinigung durch, indem Sie Checkpoints wie unten beschrieben auflisten. Wenn es mehr als einen Checkpoint gibt, bereinigen Sie ihn:
   ...
   ```
2. Führen Sie eine Offline-Komprimierung aus.  Wenn Sie nicht wissen, wie Sie die Offline-Verdichtung ausführen, lesen Sie [here](https://gist.github.com/andrewmkhoury/0b1fe4d8b619178ff87b).
3. Starten Sie den Server und warten Sie, bis die Indizierung abgeschlossen ist.
