...
In diesem Dokument werden gängige Protokolleinträge beschrieben, was sie bedeuten und wie sie behandelt werden.<br><br>[Inhaltsverzeichnis](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17490.html?lang=de)
...
Seit [!DNL dispatcher] -Modul 4.2.x verwendet haben, haben sie begonnen, Personen davon abzuhalten, den folgenden Übereinstimmungstyp in ihrer Filterdatei zu verwenden:
...
Verstehen Sie, dass die [!DNL dispatcher]-Regeln festgelegt wurden, um diese Anforderung herauszufiltern. In diesem Fall wurde die Seite, die besucht werden wollte, absichtlich abgelehnt, und wir möchten nichts daran ändern.
...
Dies tritt auf, wenn Sie die falsche IP-Adresse im Renderer-Abschnitt Ihrer Farm konfiguriert haben. Diese oder AEM Instanz reagierte nicht mehr oder hörte zu und die [!DNL dispatcher] kann es nicht erreichen.
...
Das bedeutet, dass die AEM-Instanz über einen offenen Socket verfügte, den sie erreichen konnte, und mit der Antwort eine Zeitüberschreitung aufwies. Das bedeutet, dass Ihre AEM Instanz zu langsam oder nicht gesund war und [!DNL dispatcher] hat die konfigurierten Timeout-Einstellungen im Renderer-Abschnitt der Farm erreicht. Erhöhen Sie entweder die Timeout-Einstellung oder stellen Sie sicher, dass Ihre AEM ordnungsgemäß ausgeführt wird.
...
Dieser Fehler tritt auf, wenn Sie Ihre [!DNL dispatcher] , um den dynamischen automatischen Filter zu verwenden, lassen Vanity-URLs zu, aber nicht durch die Installation des Pakets auf dem AEM Renderer abgeschlossen.
